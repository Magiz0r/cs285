...
def evaluate_policy(
    ...
    for ep_idx in range(NUM_EVAL_EPISODES):
        ...
        while not done: # when not finish recording
            if action_chunk is None or chunk_index >= chunk_size: # if actions are not generated or actions are not fully generated
                state = (
                ...
                action_chunk = normalizer.denormalize_action(pred_chunk) # action denormalization
                action_chunk = np.clip(action_chunk, action_low, action_high) # clipped numbers which are not in [action_low, action_high]
                chunk_index = 0 # reset action chunk index
